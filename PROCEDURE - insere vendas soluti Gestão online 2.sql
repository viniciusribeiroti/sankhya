create or replace PROCEDURE STP_SNK_INSEREVENDA_SOLUTI(P_ORIGEMINTEG IN VARCHAR2, --GESTAO_ONLINE, ASSINE_ONLINE, MERCADO_ON
                                                       ...
                                                       P_ORGAOPUBCLI VARCHAR2 default NULL -- CLIENTE É ORGAO PUBLICO ?
                                                       ) 
 ...
 AS
  ...
  ERR_CODE          NUMBER;
  ERR_MSG           CLOB;
  P_ORGAOPUBCLI2    VARCHAR2(40);
  P_IDPEDINTEG2     INT := 0; --VERIFICAR A INTEGRIDADE ENTRE PEDIDO E CABEÇALHO
...
BEGIN
  ...
  IF P_COUNT = 0 THEN
    ...
    -- ATUALIZA PARCEIRO QUANDO EXISTE
  ELSE
        STP_SNK_REGLOG('5_INSEREVENDA...OK: 1.0 - ATUALIZAR PARCEIRO ' || P_ORIGEMINTEG || ' - ' ||
                        P_IDPEDINTEG || ' - ' || P_CNPJEMP || ' - ' || P_CODPROJ); 

        STP_SNK_UPDATE_PAR(TO_CHAR(TRIM(REGEXP_REPLACE(P_CPF_CNPJ, '[^[:digit:]]'))),
                           UPPER(TRIM(SUBSTR(P_RAZAOSOCIAL,1,100))),
                           TRIM(P_EMAIL),
                           REPLACE(REPLACE(REPLACE(P_FONE, '(', ''), ')', ' '),'-',''),
                           P_CIDADE,
  ...
  -- DEFINIE SE É ÓRGÃO PÚBLICO
  SELECT NVL(P_ORGAOPUBCLI,'NAO') INTO P_ORGAOPUBCLI2 FROM DUAL;

  UPDATE TGFPAR SET AD_ORGAOPUBCLI = P_ORGAOPUBCLI2 WHERE CODPARC = P_CODPARC;
  ...
  FOR C1 IN (SELECT P_NUMNOTA AS NUMNOTA,
   ...
   LOOP
    ...
    WHILE P_ITENSLIST.EXISTSNODE('//item[' || P_COUNTITE || ']') = 1 LOOP
     ...
     /*****************************************************************************************************************
        ALTER.: Vinícius Ribeiro - 12/08/2021 - 15:18                                                                 *
        - COUNT para verificar se existe AD_IDPEDINTEG na TGFCAB com o NUNOTA em questão                              *
        - Se existir faz o insert na TGFITE                                                                           *
        - Se não existir atribui o valor AD_IDPEDINTEG encontrado da TGFCAB na AD_IDPEDINTEG2 e faz o insert na TGFITE*
     ******************************************************************************************************************/
     

     SELECT NVL(COUNT(1),0) INTO P_COUNTCAB FROM TGFCAB WHERE NUNOTA = P_NUNOTA AND AD_IDPEDINTEG = P_IDPEDINTEG; 

     IF (P_COUNTCAB > 0) THEN

      --INSERE ITENS com a P_IDPEDINTEG
      STP_SNK_INCLUIR_ITE(P_NUNOTA,
        ...
        STP_SNK_REGLOG('13_INSEREVENDA...OK:' || P_CODLOCAL || ' - ' || P_IDPEDINTEG);
        COMMIT;
      ELSE 
      
       SELECT AD_IDPEDINTEG INTO P_IDPEDINTEG2 FROM TGFCAB WHERE NUNOTA = P_NUNOTA;

       STP_SNK_REGLOG('13.1_INSEREVENDA...OK:'||P_IDPEDINTEG||' - '||'IDPEDINTEG DIFERENTE DA TGFCAB - '||P_IDPEDINTEG2);
    
      --INSERE ITENS com a P_IDPEDINTEG2
       STP_SNK_INCLUIR_ITE(P_NUNOTA,
                           P_SEQUENCIA,
                           P_NUTAB,
                           P_CODPROD,
       ...
       STP_SNK_REGLOG('13.1_INSEREVENDA...OK:' || P_CODLOCAL || ' - ' || P_IDPEDINTEG2);
       COMMIT;
    END IF;
